namespace CafeSportOO
{
    internal class Program
    {
        ...
        static void Bestellen(List<Bestelling> bestellingen)
        {
            ...
            if (drankNummer > 0 && drankNummer < 7)
            {
                ...
                for (int j = 0; j <= drankNummer; j++)
                {
                    if (drankNummer == j)
                    {
                        bestellingen.Add(new Bestelling(tafelNummer, Dranken[j].Naam, Dranken[j].Prijs));
                    }
                }
            }
            else
        ...
        static void Afrekenen(List<Bestelling> bestellingen)
        {
            ...
            foreach (var item in bestellingen)
            {
                ...
                {
                    Console.WriteLine($"Tafelnummer {item.Tafelnummer} heeft een {item.Naam} besteld.");
                    som = som + item.Prijs;
                }
    ...
    class Bestelling
    {
        ...
        public Bestelling(int tafelNummer, string naam, float prijs)
        {
        ...
        {
            return $"Tafel {Tafelnummer} heeft een {Naam} besteld.";
        }
    }
}
